...
let path = __dirname+'/../../system/data/status-hack.json';
let data = {};
...
let admin_tx = global.config.NDH;
...
exports.onRun = async o => {
 let {
   ...
   if (tid in d)return send('❎ Nhóm đã tạo bàn tài xỉu rồi!');
        if (sid in d.s)return(x=>send(`⚠️ Vui lòng quay lại sau ${x/1000/60<<0} phút ${x/1000%60<<0} giây, Mỗi người chỉ được tạo bàn mỗi 5 phút một lần`))(d.s[sid]-Date.now());
...
exports.onEvent = async o=> {
    ...
    let select = (t=>/^(tài|tai|t)$/.test(t)?'t': /^(xỉu|xiu|x)$/.test(t)?'x': /^(rời|leave)$/.test(t)?'l': /^infotx$/.test(t)?'i': /^xổ$/.test(t)?'o': /^(end|remove|xóa)$/.test(t)?'r': null)((args[0] || '').toLowerCase());
    let money = async id=>(await o.Currencies.getData(id)).money;
    let bet_money = args[1];
    let p;
    let Tm = (require('moment-timezone')).tz('Asia/Ho_Chi_Minh').format('HH:mm:ss | DD/MM/YYYY');
    ...
    if (select == 'o') {
        ...
        d[tid].playing = true;
        let diing = await send(`🎲 Bot đang lắc, Chờ xíu...`);/*[${diing_s}s]*/
  ...
  await send({body: `[ KẾT QUẢ TÀI XỈU ]\n────────────────────\n🎲 Xúc xắc: ${dices.join(' | ')} - tổng ${sum} điểm\n📝 Kết quả: ${select_values[winner]}\n⏰ Time: ${Tm}\n🎰 Tỉ lệ ăn 1 : ${rate}\n────────────────────\n🏆 Tổng Kết:\n▭▭▭▭▭▭▭▭\n⬆️ Những người thắng:\n${winner_players.map(($, i)=>(crease_money = $.bet_money*BigInt(String(rate)), o.Currencies.increaseMoney($.id, crease_money.toString()),`${i+1}. ${global.data.userName.get($.id)} - chọn ${select_values[$.select]}\n𖢨 Đã cộng ${crease_money.toLocaleString()}$`)).join('\n')}\n▭▭▭▭▭▭▭▭\n⬇️ Những người thua:\n${lose_players.map(($, i)=>(o.Currencies.decreaseMoney($.id, $.bet_money.toString()),`${i+1}. ${global.data.userName.get($.id)} - chọn ${select_values[$.select]}\n𖢨 Đã trừ ${$.bet_money.toLocaleString()}$`)).join('\n')}\n────────────────────\n👤 Chủ bàn: ${global.data.userName.get(d[tid].author)}`,
    ...
    if (select == 'r') {
        let dataThread = (await o.Threads.getData(tid)).threadInfo;
        if (dataThread.adminIDs.some($=>$.id == sid)) return send(`QTV đã yêu cầu kết thúc bàn tài xỉu những người đặt cược sau đây thả cảm xúc để xác nhận.\n\n${p.map(($, i)=>`${i+1}. ${global.data.userName.get($.id)}`).join('\n')}\n\nTổng cảm xúc đạt ${Math.ceil(p.length*50/100)}/${p.length} người bàn tài xỉu sẽ kết thúc.`).then(([err, res])=>(res.name = exports.config.name, res.p = p, res.r = 0, global.Seiko.onReaction.push(res)));
    }
};
