this.config = {
    ...
    role: 0,
    author: 'JRT',
    info: 'game cờ caro 5x5',
    Category: 'Game',
    guides: 'tag người muốn chơi cùng',
    cd: 5,
    hasPrefix: true,
    images: []
};
this.onReply = async ({ onReply, event, api ,Users}) => {
  function delay(ms) {
       return new Promise(resolve => setTimeout(resolve, ms));
  };
  var {x, y, d, d1, sizeboard, sectionSize, boardbuffer} = onReply;
...
this.onRun = async ({ event, api, args, msg }) => {
try {
    ...
    const spf = require("node-superfetch");
    var path1 = __dirname+'/cache/caro1'+threadID+'.png';
    var path2 = __dirname+'/cache/caro2'+threadID+'.png';    
    //==============================
    ...
    if(!fs.existsSync(__dirname+'/cache/bold-font.ttf')) {
      let getfont = (await axios.get("https://drive.google.com/u/0/uc?id=1Q4Nksy-wI_TjGhleGUPOqt4nYhudj9qC&export=download", { responseType: "arraybuffer" })).data;
    ...
    fs.writeFileSync(path2, canvasbg.toBuffer());
    msg.reply({body: "✅ Tạo ván cờ caro thành công, bạn đi trước, reply tin nhắn này kèm theo tọa độ X Y để đánh quân cờ, ví dụ:\n1 5", attachment: fs.createReadStream(path2)}, (e, info) => {
            global.Seiko.onReply.push({
                name: this.config.name,
                author: senderID,
                messageID: info.messageID,
