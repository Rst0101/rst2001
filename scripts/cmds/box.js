this.config = {
    ...
    version: "2.1.1",
    role: 0,
    author: "không biết, fix lại Tobi",
    info: "Xem thông tin thread/user",
    Category: "Box chat",
    guides: "[thread/user]",
    cd: 5,
    hasPrefix: true,
...
const totalPath = __dirname + '/../../system/data/totalChat.json';
const _24hours = 86400000;
const fs = require("fs-extra");
const request = require("request");
const axios = require("axios");
this.onEvent = async ({ api, event, args }) => {
...
this.onRun = async function({ api, event, args, Users, Threads }) {
  ...
  if (args[0] == "image") {
    ...
    if (args[0] == "info") {
        try {
            if (!fs.existsSync(totalPath)) fs.writeFileSync(totalPath, JSON.stringify({}));
            let totalChat = JSON.parse(fs.readFileSync(totalPath));
            let threadInfo = (await Threads.getData(args[1] || threadID)).threadInfo; 
