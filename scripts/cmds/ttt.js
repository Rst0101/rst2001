this.config = {
  ...
  role: 0,
  author: "Mirai Team",
  info: "Chơi caro with AI",
  Category: "Game",
  cd: 5,
  hasPrefix: true,
  guides: "x/o/delete/continue",
  images: [],
};
...
this.onReply = async function({ event, api, onReply, msg }) {
  let { body, threadID, messageID, senderID } = event;
  msg.unsend(onReply.messageID);
  ...
  if(!isNaN(number) && number > 0 && number < 10) {
    ...
    if(checkGameOver(data)) {
      ...
      if(checkAIWon(data)) lmao = `[ TIC TAC TOE ]\n\n⩺ Kết quả: Bạn thua - ${gayban[Math.floor(Math.random() * gayban.length)]}`;
      else if(checkPlayerWon(data)) lmao = `[ TIC TAC TOE ]\n\n⩺ Kết quả: Bạn thắng`;
      else lmao = `[ TIC TAC TOE ]\n\n⩺ Kết quả: Hòa`;
      global.moduleData.tictactoe.delete(threadID);
    }
    var msg = lmao !== "" ? lmao : temp == undefined ? `[ TIC TAC TOE ]\n\n⩺ Reply số ô để đánh dấu` : temp;
