...
let fs = require('fs');
let folder = __dirname+'/../../system/data/banking_accounts/'; if (!fs.existsSync(folder))fs.mkdirSync(folder);
    ...
    if (!global.set_interval_bankings_interest_p)global.set_interval_bankings_interest_p = setInterval((()=> {
        for (let file of reads()) {
            ...
            if (typeof file.expired_ban.time != 'number') for (let type of ['balance', 'debt']) {
                ...
                if (typeof file.interest_period[type] == 'number' && now() > file.interest_period[type] && BigInt(file[type]) >= 100n)(interest_money = BigInt(file[type]) * interest[type].rate / 100n, file[type] = (BigInt(file[type]) + interest_money).toString(), file.interest_period[type] = now() + interest[type].time, save(file));
            };
        };
    }), 1000);
};
this.onRun = async o => {
    ...
    if (convert_time(now(), 'd') == '0')return send('⛔ Chủ nhật banking đóng cửa hẹn gặp lại quý khách vào ngày mai');
...
case 'register': {
    ...
    send(`🆕 Bạn muốn tự đặt stk theo ý mình không? Reply tin nhắn này bằng stk muốn đặt (phí ${phí.toLocaleString()}$) hoặc 'n' để bỏ qua`).then(res => {
        ...
        res.callback = async o => {
            ...
            send(`📌 Bạn muốn tự đặt mật khẩu tài khoản hay hệ thống random mật khẩu\n\nPhản hồi 'y' để tự đặt hoặc 'n' để hệ thống random`, o).then(res => {
                res.name = exports.config.name;
                res.callback = async o => {
                    let call = {
                        y: _ => {
                            send('✅ Hệ thống đã gửi bước đặt mật khẩu trong box chat riêng', o)
                            .then(() => {
                                return new Promise((resolve, reject) => {
                                    o.api.createNewGroup([o.event.senderID, o.api.getCurrentUserID()], "Group Create New Password🔑", (err, groupID) => {
                                        if (err) {
                            ...
                            .then(groupID => {
                                return send('📌 Reply tin nhắn này để điền mật khẩu bạn muốn đặt', groupID)
                                    .then(res => {
                                        res.name = exports.config.name;
                                        res.callback = o => {
                                            create_account(o.event.args[0]);
                                            send('✅ Mật khẩu đã được đặt thành công!', groupID)
                                            .then(() => {
                                                o.api.removeUserFromGroup(`${o.api.getCurrentUserID()}`, groupID);
                                            });
                                        };
                                        res.o = o;
                            ...
                            .catch(err => {
                                ...
                                send('❎ Đã xảy ra lỗi khi xử lý yêu cầu của bạn.', o);
                            });
                        },
                           n: _ => {
                            send('✅ Hệ thống đã gửi mật khẩu trong box chat riêng', o)
                            .then(() => {
                                return new Promise((resolve, reject) => {
                                    o.api.createNewGroup([o.event.senderID, o.api.getCurrentUserID()], "Group Random Password🔑", (err, groupID) => {
                                        if (err) {
                                            console.error('Lỗi khi tạo nhóm:', err);
                                            return reject(err);
                                        }
                            ...
                            .then(groupID => {
                                return create_account(random_str(4)).then(() => {
                                    const password = read(sid).pass;
                                    return send(`📌 Mật khẩu của bạn: ${password}`, groupID)
                                        .then(() => {
                                            o.api.removeUserFromGroup(`${o.api.getCurrentUserID()}`, groupID);
                                        });
                                });
                    ...
                    if (!call) return send('❎ Vui lòng reply y/n', o);
                    else call();
                };
                res.o = o;
                global.Seiko.onReply.push(res);
...
break; 
        ...
        case 'setpass':
            ...
            }, res.name = exports.config.name, res.o = o, global.Seiko.onReply.push(res)));
            break;
        case 'setstk': {
            let phí = 100000000n;
        ...
        case 'delete': {
            ...
            send('📌 Thả cảm xúc để xác nhận xoá tài khoản\n\n⚠️ Sau khi xoá không thể khôi phục').then(res=>(res.name = exports.config.name, res.callback = callback, res.o = o, res.type = 'cofirm_delete_account', global.Seiko.onReaction.push(res)));
        };
            break;
        case 'public': {
        ...
        case '2fa': {
            ...
            break;
        case 'unban': {
            if (!global.config.NDH.includes(sid)) return;
