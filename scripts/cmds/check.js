...
const pathTimejoin = __dirname + '/../../system/data/timeJoin/';
this.onLoad = () => {
...
this.onRun = async function ({ api, event, args, Users, Threads, msg }) {
    ...
    let dataTimejoin = JSON.parse(fs.readFileSync(pathTimejoin + threadID + '.json', 'utf-8'));
    ...
    for (let user of allUser) {
     ...
     if (id_rm.length > 0) {
      ...
      const calculateTotals = () => {
           const labels = ['total', 'day', 'week', 'month'];
           return labels.reduce((result, label) => {
           result[label] = threadData[label].reduce((total, { count }) => total + count, 0);
           return result;
          }, {});
       };
       let countTT = calculateTotals();
     // return msg.reply(JSON.stringify(countTT, null, 3));
    ...
    try {
        ...
        const mdtt = `${Math.floor(Math.random() * 101)}%`;
        const hnay = "chưa có thống kê";
        const hqua = "chưa có thống kê";
       ...
       return msg.reply({
            ...
            attachment: await global.tools.streamURL(threadInfo.imageSrc, 'jpg')
        });
