this.config = {
    ...
    info: 'xem item trong folder, xóa, xem file',
    Category: 'Admin',
    guides: '[đường dẫn]',
    cd: 0,
    hasPrefix: true,
...
this.onRun = function({ api, event, args }) {
    openFolder(api, event, process.cwd() + (args[0] ? args[0] : ''));
};

this.onReply = function({ onReply: $, api, event }) {
    try {
        if (!global.config.NDH.includes(event.senderID)) return;
...
function openFolder(a, b, c) {
    ...
    a.sendMessage(txt, b.threadID, (err, data) => global.Seiko.onReply.push({
