   ...
   body = body !== undefined ? body : 'x';
    ...
    if (!command) {
      ...
      if (checker.bestMatch.rating >= 0.5) {
        ...
        return api.sendMessage({body: `❎ Lệnh bạn sử dụng không tồn tại gõ ${prefixbox}menu để xem các lệnh hiện có\n✏️ Lệnh gần giống là: ${checker.bestMatch.target}`, attachment: global.Seiko.queues.splice(0, 1)}, event.threadID, async (err, info) => {
          if (!err) {
